...
public class ChampionshipController {
	...
	public ChampionshipController(ChampionshipService championshipService, ChampionshipResponseModelAssembler championshipResponseModelAssembler) {
	...
	@PostMapping
	@ApiResponse(responseCode = CommonUtil.HTTP_STATUS_CODE_CREATED, description = "Season started successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ChampionshipResponse.class)))
	@ApiResponse(responseCode = CommonUtil.HTTP_STATUS_CODE_CONFLICT, description = "Championship is already registered", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ConflictResponse.class)))
	...
	public ResponseEntity<Object> register(@RequestBody @Valid ChampionshipRegisteringRequest championshipRegisteringRequest, BindingResult bindingResult) {
		...
		championshipRegisteringRequest.setDivisionName(championshipRegisteringRequest.getDivisionName().toUpperCase());

		ChampionshipResponse cResponse = this.championshipService.findByNameAndDivisionName(championshipRegisteringRequest.getName(), championshipRegisteringRequest.getDivisionName());
		if (cResponse != null) {
